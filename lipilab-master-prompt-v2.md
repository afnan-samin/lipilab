...
- **Do not architect production pricing or capacity planning around this specific free model.** OpenRouter's `:free` model variants are provider-controlled and are commonly rotated, rate-limited, or discontinued without notice — at least one tracking source indicates this exact model's free availability has already been inconsistent. The backend's model selection must be a **config value, not a hardcoded string**, so swapping models requires an admin-config change, not a code change.
- **Recommended production model: `google/gemini-3.1-flash-lite` (GA, not the deprecated Preview variant — the Preview was shut down 2026-05-25).** Reasoning: ranked well on an informal Bengali-language leaderboard (treat this as a weak signal, not a rigorous benchmark — the site is a community aggregator, not peer-reviewed), very low cost ($0.25/M input, $1.50/M output tokens), 1M+ context window (comfortable for MCQ batch classification), and supports structured/JSON output (needed for reliable MCQ metadata extraction).
- **Cost-modeling note:** Bangla script has higher token-fertility than English in most tokenizers (roughly 2–3 tokens per word rather than ~1) per current NLP literature — factor this into the per-word point-rate formula (§1.2), not a flat 1-token-per-word assumption. Even accounting for this, back-of-envelope cost at the recommended model's pricing is roughly $0.00000075/word — a wide margin under any reasonable point-rate, but the tokens-per-word multiplier should still be measured against real sample text, not assumed.
- **Before committing to production**, empirically test 2–3 candidate models (e.g. `google/gemini-3.1-flash-lite`, `openai/gpt-5.4-mini`, the paid `deepseek/deepseek-v4-flash`) against real Bijoy-converted Bangla/English mixed sample text with deliberately seeded spelling errors — general Bengali leaderboards measure broad content/translation tasks, not this app's narrow mixed-script spell-detection task specifically.
...
### 1.6 Worked Cost Example — Spell Check Advanced (for point-rate calibration)
Using `google/gemini-3.1-flash-lite` pricing ($0.25/M input, $1.50/M output) and ~123 BDT/USD:

| Scenario | Assumptions | Cost per 100,000 words |
|---|---|---|
| Low | 2 tokens/word, 2% error rate, terse JSON output | ≈ ৳12 |
| Mid | 2.5 tokens/word, 5% error rate | ≈ ৳19 |
| High | 3.5 tokens/word, 8% error rate, verbose/explained output | ≈ ৳62 |

**Key takeaway:** output tokens cost 6× input tokens at this pricing, so prompt-output verbosity (explanations vs. terse `{word, position, suggestion}` JSON) is the single biggest cost lever — keep output minimal by design. The two-tier dictionary-prefilter (§2 of the prior expansion prompt) should also reduce actual LLM input volume well below the full document in typical use, since only dictionary-uncertain sentences need to reach the LLM.
**Implication for §1.2:** Spell Check Advanced's point-rate must be calibrated against the **High scenario**, not Low/Mid, to avoid a loss-making price — do not reuse the conversion feature's ~50 BDT/100K-word price point for this feature; treat them as independently priced. Validate with real token-usage data (visible per-call in the OpenRouter dashboard) from actual test documents before finalizing the rate — this table is a planning estimate, not measured data.
...
- **v2.2 → v2.3**: Corrected the free-path ad mechanic (§1.0a) — since conversion is near-instant, a mandatory ad per batch produced back-to-back ads with no real work between them. Changed to a cooldown-between-batches model with an optional "watch ad to skip the wait" button, preserving user choice and avoiding ad-network abuse patterns. Recorded current LLM testing setup (OpenRouter, `deepseek/deepseek-v4-flash:free`) and flagged that production must not depend on this specific free model staying free/available (§1.5). Added a concrete production model recommendation (`google/gemini-3.1-flash-lite` GA) with pricing, and a Bangla token-fertility caveat for the point-rate cost formula.
- **v2.3 → v2.4**: Added a worked cost example (§1.6) for Spell Check Advanced at ~100,000 words, showing cost ranges from ৳12 (low) to ৳62 (high, verbose-output scenario) — clarified that conversion's near-zero LLM cost and Spell-check/MCQ's real per-token cost are separate pricing problems and must not share a price point. Flagged output-token verbosity as the primary cost lever to control via prompt design.
